...
3. データの整合性を保証するgetter/setterメソッドを追加（例：学生番号は変更不可、成績は0-100の範囲など）
...
入力検証、ファイル操作、データベース接続などの箇所に例外処理を追加し、エラー発生時には具体的なエラーメッセージの表示、ログ出力、データの整合性維持、リトライ処理などの対処と回復処理を実装してシステムの堅牢性を高めます。
...
3. 例外処理とエラーハンドリング
   - カスタム例外の設計
   - ユーザー入力エラー（成績範囲外、空文字、不正な形式）、ファイル読み書きエラー（権限不足、ディスク容量不足）、データ不整合（重複ID、参照整合性の違反）などに対する具体的な例外処理の実装
   - ロギングの活用
...
#### フェーズ3-4関連
- B.08「コレクションの内部実装」: ArrayListとLinkedListの時間計算量の違いを理解し、データアクセスパターンに応じた最適なデータ構造選択
