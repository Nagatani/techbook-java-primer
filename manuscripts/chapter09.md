...
前章まで、Javaにおけるオブジェクト指向プログラミングの核心技術について学習してきました。本章では、Java 8（2014年）で導入された革命的な機能である「ラムダ式（Lambda expressions）」と「関数型インターフェイス（Functional interfaces）」について詳細に学習していきます。
...
**論理型プログラミング（1970年代〜）**：論理的推論にもとづてプログラムを記述する手法で、Prologなどで実装されました。人工知能や知識表現の分野で活用されています。
