...
パッケージは、関連するクラスやインターフェイスをグループ化するしくみです。名前空間を提供し、クラス名の衝突を防ぎます。
...
## 🚀 基礎統合プロジェクト：図書館管理システム

第5章までの学習内容を統合して、実用的なアプリケーションを開発してみましょう。このプロジェクトは、オブジェクト指向設計の実践的な体験を提供します。

### プロジェクト概要

**システム名**：図書館管理システム
**対象技術**：第0章〜第5章で学習したすべての内容
**開発期間目安**：1-2週間
...
2. **継承の適切な活用**
   - 共通属性を持つクラス（Person → User, Staff）
   - 資料タイプの階層（Material → Book, Magazine, DVD）
...
このプロジェクトが完成したら、第6章以降の技術（例外処理、コレクション、ジェネリクス等）を段階的に導入して、より実用的で堅牢なシステムに発展させていくことができます。
...
8. **Javadocによるドキュメント**: ソースコードから自動的にドキュメントを生成する
