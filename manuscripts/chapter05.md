...
本章で学ぶ「ポリモーフィズム（多態性）」は、**総合演習プロジェクト「ToDoリストアプリケーション」** において、さまざまな種類のタスクを統一的に扱うための鍵となる概念です。

- **多様なタスクの表現**: 今は単純なタスクしかありませんが、将来的には「締め切り付きタスク `DeadlineTask`」や「繰り返しタスク `RecurringTask`」など、新しい種類のタスクを追加したくなるかもしれません。
- **統一的な管理**: これらの多様なタスクは、すべて「タスク」という共通の性質を持っています。ポリモーフィズムを利用することで、`ArrayList<Task>` のような単一のリストで、これらの異なる種類のタスクをすべて一緒に管理できます。
- **柔軟な処理**: リストをループ処理する際、個々のオブジェクトが `DeadlineTask` なのか `RecurringTask` なのかをいちいち気にする必要はありません。`task.display()` のように呼びだすだけで、Javaが自動的に各オブジェクトの正しい`display`メソッドを呼び分けてくれます。

本章を通じて、具体的な型に縛られず、より抽象的な「役割」に着目してプログラミングを行うことで、柔軟で拡張性の高いコードを記述する技術を学びます。
...
これにより、子クラスは親クラスの機能を再利用しつつ、新しい機能を追加し��り、既存の機能を変更（オーバーライド）したりできます。
...
public class Knight extends Character {
    ...
    public void attack() {
        ...
        System.out.println("追加で��を振るった！"); // 子クラス独自の処理を追加
    }
}
```
...
**目的:** 継承、オーバーライド、ポリモーフィズムを使って、異なる種類の従業員の給与計算を統一的に扱うプログラムを作成���る。
