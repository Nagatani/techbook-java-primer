...
本章を学習するために、いくつかの前提知識があるとスムーズに進められます。まず技術的な前提として、C言語での基本的なプログラミング経験があるとよいでしょう。具体的には、変数の宣言や操作、関数の定義と呼び出し、if文やforループなどの制御構造を理解し、実際にプログラムを作成した経験があることを想定しています。また、コンパイルと実行の基本概念を理解していると、Javaのコンパイル方式の違いをより深く理解できます。さらに、基本的なコンピュータ操作、特にファイルの作成やディレクトリの操作に慣れていることも助けになります。

概念的な前提として、プログラムとは何か、つまりコンピュータに対して特定の処理を実行させるための指示の集まりであることを理解していると、学習がスムーズに進みます。また、データと処理の概念、すなわちプログラムが操作する対象（データ）と、それに対して行う操作（処理）の関係を理解していることも助けになります。さらに、問題解決の論理的思考、つまり複雑な問題を小さな部分に分割し、順序立て解決する能力があることが、Javaでのオブジェクト指向プログラミングを学ぶ上で大きな助けとなります。

### 学習目標

本章では、Javaプログラミングの学習を始めるにあたって必要な基礎知識と環境構築を学びます。知識理解の面では、まずJava言語の歴史的背景と設計思想を探っていきます。Javaは1995年にSun Microsystems社によって開発され、「Write Once, Run Anywhere」という革新的な概念を提唱しました。この背景には、当時のソフトウェア開発が抱えていたプラットフォーム依存性の問題があり、Javaがどのようにこの問題を解決したかを見ていきましょう。

C言語からJavaへの移行の意義を理解することも大切な学習目標です。C言語はシステムプログラミングに優れた言語ですが、大規模なアプリケーション開発においては、メモリ管理の複雑さ、モジュール化の困難さ、プラットフォーム依存性などの課題があります。Javaはこれらの課題を、ガベージコレクション、オブジェクト指向、仮想マシンなどの機能で解決しています。

オブジェクト指向の概念的な理解も本章の大切な目標です。オブジェクト指向プログラミングは、データとそれを操作する処理を1つの単位（オブジェクト）としてまとめるプログラミングパラダイムです。このデータと処理の一体化がもたらす利点、たとえばコードの再利用性の向上、保守性の改善、現実世界のモデル化の容易さなどを理解することが、Java学習の基礎となります。
...
また、手続き的なプログラミングから一歩進んで、オブジェクト指向プログラミングを学びたい方にもぴったりです。オブジェクト指向は、大規模なソフトウェア開発において非常に役立つ考え方で、現代のソフトウェア開発の主流となっています。
