...
#### **第5段階：実践的な開発スキル**（第21章〜第23章）
**学習期間目安：2-3週間**

- **第21章：ユニットテストと品質保証** - テスト駆動開発とJUnit
- **第22章：ドキュメントと外部ライブラリ** - Javadocと外部ライブラリの活用
- **第23章：ビルドとデプロイ** - MavenとGradleによるプロジェクト管理
...
- **外部連携技術群**：第14章（例外処理）→ 第15章（ファイルI/O）→ 第17章（ネットワーク）→ 第22章（外部ライブラリ）
...
プログラミング学習において、エラーは避けて通れない重要な学習機会です。初学者の多くはエラーメッセージを見て挫折してしまいますが、エラーは「プログラムが期待通りに動かない理由を教えてくれる貴重な情報源」として捉えるべきです。

#### エラーメッセージの例と読み方

たとえば、文字列を数値に変換する際によく遭遇するエラーを見てみましょう：
...
1. **エラーの種類**: `NumberFormatException` - 数値形式の例外
2. **エラーの原因**: `For input string: ""` - 空文字列を数値に変換しようとした
3. **エラーの発生場所**: スタックトレースから、どこでエラーが起きたかがわかる

#### エラーへの対処法の考え方

このようなエラーに遭遇したとき、以下のステップで対処します：

1. **エラーメッセージを落ち着いて読む** - 何が問題なのかを理解する
2. **原因を特定する** - なぜそのエラーが発生したのかを考える
3. **解決策を検討する** - 入力値の検証、エラー処理の追加などを考える

現時点では、すべてのエラーに対処する必要はありません。エラー処理や例外処理の詳細は第14章で学習します。今は「エラーは学習の機会」という心構えを持つことが大切です。

#### 第1章での学習の焦点

本章の学習段階では、以下の点を理解しておけば十分です：

**現時点で考える必要がないこと**：
- プログラムが「整数値を入力してください」と表示したとき、ユーザーが文字や記号を入力する可能性
- 空文字列や想定外の形式での入力への対処
- あらゆる異常入力に対する防御的なプログラミング

**なぜ考えなくて良いのか**：
1. **段階的な学習が重要** - まずは正常な動作を理解することが先決
2. **基本を固めてから応用へ** - エラー処理は基本文法を習得してから学ぶべき高度な技術
3. **練習課題の目的** - 第1章の課題は「正しく入力された場合の処理」を学ぶことが目的

つまり、「整数値を入力してください」という指示に対して、学習者が正しく整数を入力することを前提として練習を進めて構いません。実際のアプリケーション開発では入力検証が必須ですが、それは後の章で体系的に学習します。

> **重要な心構え**  
> プログラミングのスキルは、エラーを解決する経験を積むことで向上します。エラーメッセージは敵ではなく、問題解決のヒントを与えてくれる味方です。恐れずに、一つひとつ理解しながら進んでいきましょう。
> 
> 第1章では、まず「プログラムが正しく動く喜び」を体験することを優先し、エラー処理の複雑さに圧倒されないようにしましょう。
...
**付録リソース**: `/appendix/b01-jvm-architecture/`
