...
- **カプセル化によるデータ保護**: `Task`クラスが持つべき情報（タスクの内容、完了状態など）を`private`なフィールドとして隠蔽します。外部からは`public`なメソッド（`getTaskName()`や`setCompleted(boolean)`)を通じてのみアクセスできるようにすることで、意図しないデータの書き換え��防ぎ、`Task`オブジェクトが常に正しい状態を保つようにします。
...
### 前提知識
- **第3章のクラスとオブジェクトの基本概念の理解**：クラス、フィールド、メソッド、コンストラクタの作成経験
- **基本的なJavaプログラムの作成経験**：複数のクラスを組み合わせたプログラムの作成経験

### 到達目標

#### 知識理解目標
- カプセル化の目的と重要性の理解
- 4種類のアクセス修飾子（`public`, `protected`, `default`, `private`）の違いと役割の理解
- パッケージの役割と命名規則の理解
- `import`文の役割と、完全限定名との違いの理解

#### 技能習得目標
- `private`フィールドと`public`なgetter/setterメソッドによるカプセル化の実装
- ビジネスルールをsetterメソッドに実装し、データの整合性を維持する技術
- 複数のパッケージを持つプロジェクトの作成と管理
- `import`文を適切に使い、他パッケージのクラスを利用する技術
...
| :--- |
| 役割にもとづいたパッケージ分割ができる |
| すべてのクラスで適切なアクセス修飾子を使い、カプセル化を実践できる |
| 外部ライブラリのクラスを`import`して利用できる |
| パッケージ名やクラス名の衝突が起きた際に、原因を理解し解決できる |
...
*   **迷ったら、できるだけ厳���い（狭い）範囲の修飾子を選ぶ**: 最初は `private` にしておき、必要に応じてアクセス範囲を広げていくのが安全なアプローチです。
...
*   **名前空間の提供**: パッケ���ジが異なれば、同じ名前のクラスを定義できます。これにより、Java標準ライブラリのクラス名（例： `List`）や、外部ライブラリのクラス名と偶然同じ名前を付けてしまっても、衝突を避けられます。クラスの完全な名前は `パッケージ名.クラス名` となります（例： `java.util.List`）。
...
この`package`宣��は、ソースファイルの**物理的なディレクトリ構造と一致している必要があります**。`com.example.geometry`パッケージの場合、ソースファイルは以下のようなディレクトリに配置されている必要があります。
...
他のパッケージに属するクラスを利用す��には、本来`パッケージ名.クラス名`という**完全限定名 (Fully Qualified Name)** で記述する必要があります。
...
このような場合は、片方を`import`し、もう片方は完全限��名で記述して、どちらのクラスを使うかを明示的に指定する必要があります。
