...
コレクションフレームワークを安全に使う上で欠かせないのが**ジェネリクス（Generics）** です。ジェネリクスは、コレクションが「どの型のデータ」を格納するのかを、コンパイル時に明確に指定するためのしくみです。
...
`ArrayList`は、`List`インターフェイスの最も代表的な実装クラスです。内部的に配列を使っており、要素へのランダムアクセス（インデックス指定での取得）が非常に高速です。
...
`HashSet`は、`Set`インターフェイスの最も代表的な実装クラスです。ハッシュというしくみを使い、要素の追加や検索を非常に高速に行います。
...
`HashMap`は、`Map`インターフェイスの最も代表的な実装クラスです。`HashSet`と同様にハッシュを使い、キーによる高速な値の検索を実現します。
