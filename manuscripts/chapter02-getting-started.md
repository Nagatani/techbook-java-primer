...
実践的なJavaプログラミングでは、以下の4つの型で大部分のケースに対応できる。
...
残りの型（`byte`、`short`、`long`、`float`）は、特殊な要件がある場合にのみ使用する。
...
参考として、Javaの8つのプリミティブ型の完全な仕様を示します。
...
実行結果（入力例：名前に「田中太郎」、年齢に「abc」（エラー）→「25」を入力）：
```
Hello, World!
改行なし出力
整数: 10, 浮動小数点: 3.14
