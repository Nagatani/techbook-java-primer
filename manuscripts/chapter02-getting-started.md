...
実際のプログラミングスキルとして、まずJavaの基本データ型（int、double、booleanなど）を目的に応じて使い分ける能力を身につけます。これには、各データ型の特性、メモリ使用量、および最適な使用場面の理解が含まれます。
...
メソッドの定義、呼び出し、戻り値の処理を習得し、コードの再利用性と可読性を向上させる手法を学びます。また、配列の宣言、初期化、操作を通じて、複数のデータを一括で扱えるようになります。
...
| 優先順位 | 演算子 | 説明 | 結合性 |
|---------|--------|------|--------|
| 1 | `()` `[]` `.` | 括弧、配列アクセス、メンバーアクセス | 左→右 |
| 2 | `++` `--` (後置) | 後置インクリメント/デクリメント | 左→右 |
| 3 | `++` `--` (前置) `+` `-` `!` `~` | 単項演算子 | 右→左 |
| 4 | `*` `/` `%` | 乗算、除算、剰余 | 左→右 |
| 5 | `+` `-` | 加算、減算 | 左→右 |
| 6 | `<<` `>>` `>>>` | シフト演算 | 左→右 |
| 7 | `<` `<=` `>` `>=` `instanceof` | 比較演算 | 左→右 |
| 8 | `==` `!=` | 等価演算 | 左→右 |
| 9 | `&` | ビットAND | 左→右 |
| 10 | `^` | ビットXOR | 左→右 |
| 11 | `|` | ビットOR | 左→右 |
...
> **注意**: 複雑な式では括弧を使用して意図を明確にすることを推奨します。コンパイラの最適化により、適切な括弧の使用がパフォーマンスに悪影響を与えることはありません。
...
- 簡潔な記述`->`記法により、コードがより読みやすく保守しやすくなる
...
- 拡張for文の利便性： 配列やコレクションの全要素を処理する際に、インデックス管理が不要で安全である。Java 5で導入されたこの機能により、境界値エラーによるバグがほぼ解消されました
...
- lengthプロパティC言語では配列のサイズを別途管理しますが、Javaでは`array.length`で常に正確なサイズを取得できる
...
1. サイズの固定性：一度作成した配列のサイズは変更できません。動的にサイズを変更したい場合は、後の章で学習する`ArrayList`などのコレクションを使用する。

2. 参照型としての配列：配列は参照型ですので、配列変数の代入は参照のコピーになる。
...
- 戻り値の型宣言メソッドが返す値の型を明示することで、呼び出し側で正しい型の変数で結果を受け取れる
...
- デバッグの容易性問題が発生した場合、特定のメソッド内に原因を特定しやすくなる
- 可読性の向上メソッド名により処理内容が明確になり、プログラムの意図が理解しやすくなる
...
Javaの文字列設計におけるもっとも重要な特徴の1つが不変性（Immutability）です。この設計思想は、最初は直感に反するように感じることがありますが、スレッドセーフティ、セキュリティ、パフォーマンスの観点から極めて重要な役割を果たしています。文字列が不変であることにより、複数のスレッドが同じ文字列オブジェクトを安全に共有でき、予期しない変更によるバグを防ぐことができます。以下のコード例で、この重要な概念を実際に確認してみましょう。
