# 第2章 Java基本文法<small>変数・制御構造・メソッドの基礎</small>

## 本章の学習目標

### 前提知識

本章でJavaの基本文法を学習するために、いくつかの前提知識があるとスムーズに進められます。

**基本的な前提知識**：

まず、第1章で学んだJava開発環境の構築が完了していることがポイントです。これには、JDKのインストール、IDEまたはテキストエディタの設定、そしてコマンドラインツールの動作確認が含まれます。

次に、Hello Worldプログラムの作成と実行を成功させた経験があるとよいでしょう。この経験を通じて、Javaプログラムのコンパイルと実行の基本的なワークフローを理解していることが前提となります。

さらに、C言語での基本的なプログラミング経験があることを想定しています。具体的には、変数の宣言と代入、if文やswitch文による条件分岐、for文やwhile文によるループ処理の経験です。これらの構造化プログラミングの基礎があることで、Javaの文法との比較を通じて、より深い理解が得られます。
...
また、コマンドライン操作の基礎知識もあると役立ちます。Javaのコンパイルや実行はコマンドラインから行うことが基本であり、ディレクトリの移動（cdコマンド）、ファイル一覧（lsまたはdirコマンド）などの基本操作を理解していることが学習を加速させます。
...
Javaは**オブジェクト指向を中心とした言語**として設計されていますが、実際には**マルチパラダイム言語**です。すべてのコードはクラスの中に記述されますが、手続き型プログラミング、関数型プログラミング、ジェネリックプログラミングなど、さまざまなプログラミングスタイルをサポートしています。この柔軟性により、問題に応じて最適なアプローチを選択できます。
