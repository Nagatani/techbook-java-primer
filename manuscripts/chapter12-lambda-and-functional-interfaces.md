...
- `(int a, int b) -> ...` は `(a, b) -> ...` と書けます
- `(a) -> ...` は `a -> ...` と書けます
- `a -> { return a * 2; }` は `a -> a * 2` と書けます
- `() -> System.out.println("Hello");` のように括弧だけを書きます
...
- ラムダ式は、匿名関数を簡潔に記述するための構文で、冗長な匿名クラスを置き換えます
- ラムダ式は、抽象メソッドが1つだけの関数型インターフェイスとして扱われます
- `Predicate`, `Function`, `Consumer`, `Supplier`など、汎用的な関数型インターフェイスが標準で用意されています
- メソッド参照を使うと、既存のメソッドを呼びだすだけのラムダ式をさらに簡潔に書けます
