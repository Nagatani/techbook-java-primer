...
- **`Task`クラスの設計**: ToDoリストの一つ一つのタスクを、単なる文字列ではなく、「タスク」というモノ（オブジェクト）として捉えます。この`Task`オブジェクトが持つべき情報（タスク名、完了状態など）と、できること（完了状態を切り替えるなど）をクラスとして設計します。
- **カプセル化**: `Task`オブジェクトの状態が、意図しない形で変更されるのを防ぎます。例えば、「完了状態は、`setCompleted()`メソッドを通さなければ変更できない」といったルールを設けることで、プログラムの安全性を高めます。
