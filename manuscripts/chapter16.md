...
#### 知識理解目標
- 単体テストの重要性と、それがもたらす4つのメリット（早期バグ発見、安全なリファクタリング、動く仕様書、開発サイクルの高速化）を説明できる。
...
- テスト用の偽物（スタブ）を作成し、依存関係を差し替えて単体テストを実行できる。
...
ソフトウェアの品質を保証するための最も基本的で強力な活動が「**単体テスト（ユニットテスト）**」です。単体テストとは、ソフトウェアを構成する**最小単位**（クラスやメソッドなど）が、それぞれ意図通りに正しく動作するかを検証するテストです。

単体テストは、単にバグを見つけるだけの消極的な作業ではありません。品質の高いソフトウェアを効率的に生み出すための、積極的な活動であり、主に4つの大きなメリットがあります。

1.  **バグの早期発見**: 開発の初期段階でバグを発見できれば、修正コストは最小限で済みます。コードを書いた直後に品質を検証することで、手戻りを劇的に減らします。
2.  **変更を恐れないための「安全網」**: 機能追加や性能改善のために既存のコードを修正（リファクタリング）する際、単体テストがあれば「変更によって既存機能が壊れていないか」を瞬時に確認できます。これにより、開発者は安心してコードの改善に挑戦できます。
...
4.  **開発サイクルの高速化**: 手動での動作確認は時間がかかり、ミスも発生しやすい作業です。自動化された単体テストは、何百もの検証を数秒で完了させ、開発のリズムを向上させます。
...
`Calculator`のような、他に依存しない自己完結したクラスは非常にテストしやすいです。
...
問題は、クラスが他のクラスに**依存**している場合です。特に、あるクラスが別の**���体的なクラスを内部で直接生成している状態（密結合）** は、テストを著しく困難にします。
...
この`UserService`を単体テストしようとすると、`new UserRepository()`によって、必ずデータベース接続処理が実行されてしまいます。これでは、`UserService`のロジック（文字列を連結する処理）だけを**独立して検証できません**。これが密結合がもたらす問題です。
...
本章では、ソフトウェアの品質を支える単体テストと、それを可能にする疎結合設計（DI）について学びました。

- **単体テスト**は、バグの早期発見や安全なリファクタリングを可能にする、品質保証��基盤です。
