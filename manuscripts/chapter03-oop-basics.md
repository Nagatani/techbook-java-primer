...
第2章でJavaの基本文法を理解していれば準備完了です。手続き型プログラミングの限界を感じたほうには、オブジェクト指向が新しい視点を提供します。



## 章の構成

本章では、オブジェクト指向プログラミングの基本的な考え方と実装方法を学習します。プログラミングパラダイムの歴史的な発展を理解したうえで、以下の内容を順に学んでいきます。
...
オブジェクト指向は、プログラミングの歴史のなかで生まれた多くのアプローチの1つです。本書では、オブジェクト指向を特別視するのではなく、問題解決のための便利なツールの1つとして学びます。
...
C言語でも不可能ではありませんが「もっと効率よくしたい」という要望のために、オブジェクト指向という考え方が登場します。
...
static修飾子は強力な機能ですが、オブジェクト指向プログラミングの基本原則と相反する場合があります。適切な場面で適切に使用することで、よりよい設計のプログラムを作成できます。第7章以降では、インターフェイスや抽象クラスと組み合わせた、より高度なstatic活用パターンを学習します。
