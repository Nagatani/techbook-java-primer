...
※本書執筆時点のLTS版（長期サポート対応バージョン）のなかで最も新しいものを使用しています。
...
1. ターミナルを起動する
    - macOSのターミナルソフトウェアが起動すればよい。ターミナルソフトウェアがわからなければ、以下の操作で起動するウィンドウを使用する
...
テキストエディタは何でもよいのですが、ファイル名「```HelloWorld.java```」として、ファイルを新規作成しましょう。
